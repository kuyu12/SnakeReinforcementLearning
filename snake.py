...
class Snake:
    ...
    def checkCrashWithDirection(self , direction):
        if direction == self.direction:
            return False
        
        if direction == KEY["UP"] and self.direction != KEY["DOWN"]:
            copy_snake = self.copy()
            copy_snake.setDirection(KEY["UP"])
            copy_snake.move()
            return copy_snake.checkCrash()
        if direction == KEY["DOWN"] and self.direction != KEY["UP"]:
            copy_snake = self.copy()
            copy_snake.setDirection(KEY["DOWN"])
            copy_snake.move()
            return copy_snake.checkCrash()
        if direction == KEY["LEFT"] and self.direction != KEY["RIGHT"]:
            copy_snake = self.copy()
            copy_snake.setDirection(KEY["LEFT"])
            copy_snake.move()
            return copy_snake.checkCrash()
        if direction == KEY["RIGHT"] and self.direction != KEY["RIGHT"]:
            copy_snake = self.copy()
            copy_snake.setDirection(KEY["RIGHT"])
            copy_snake.move()
            return copy_snake.checkCrash()
        return False
        
    def copy(self):
        copy_snake = Snake(self.x,self.y)
        copy_snake.direction = self.direction
        copy_snake.stack = self.stack
        return copy_snake
...
def main():
    ...
    while counter_games < 100:
        ...
        max_apples = 1
        eaten_apple = False
        apple = Apple(random.randint(60,SCREEN_WIDTH),random.randint(60,SCREEN_HEIGHT),1)
#        respawnApples(apples,max_apples,mySnake.x,mySnake.y)
        ...
        while(endgame!=1):
            ...
            checkLimits(mySnake)
            iscrash = mySnake.checkCrash()
            
            if(checkCollision(mySnake.getHead(),SNAKE_SIZE,apple,APPLE_SIZE)==True):
                mySnake.grow()
                score+=1
                eaten_apple=True
                
            state_old = agent.get_state(mySnake, apple)
...
#            perform random actions based on agent.epsilon, or choose the action
            if randint(0, 200) < agent.epsilon:
                ...
                prediction = agent.model.predict(state_old.reshape((1,input_node)))
            ...
            if np.array_equal(final_move,[0, 1, 0, 0]):
                mySnake.setDirection(2)
            if np.array_equal(final_move,[0, 0, 1, 0]):
                mySnake.setDirection(3)
            if np.array_equal(final_move,[0, 0, 0, 1]):
                mySnake.setDirection(4)
            if(keyPress):
                mySnake.setDirection(keyPress)  
            mySnake.move()
            
            is_current_eat = eaten_apple
            if eaten_apple:
                print("Eaten Apple")
                apple = respawnApple(0,mySnake.getHead().x,mySnake.getHead().y)
                eaten_apple = False
    
            state_new = agent.get_state( mySnake, apple)
            reward = agent.set_reward(is_current_eat, iscrash)
            ...
            record = get_record(score, record)
